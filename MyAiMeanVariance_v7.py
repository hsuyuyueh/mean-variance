...
MARKET_SOURCES = {
    "TW": lambda: [tk for tk, _ in {**dict(fetch_0050_components()),
                                    **dict(fetch_00713_components()),
                                    **dict(fetch_0056_components())}.items()
                   if not tk.startswith("289")],
    "US": lambda: [tk for tk, _ in {**dict(fetch_US_SPY_components())}.items()],
...
class AiMeanVariancePortfolio:
    def __init__(self, tickers, prices, market, profile_level='P3'):
...
if __name__ == '__main__':
    ...
    prices  = yf.download(tickers, start=start, end=end, auto_adjust=False)["Adj Close"].dropna(axis=1, how="all")
    #model   = AiMeanVariancePortfolio(tickers, prices, profile_level='P4')
